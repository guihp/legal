# 🔐 PLANO DE IMPLEMENTAÇÃO DE SEGURANÇA — IAFÉ IMOBI
...
Este documento é a **fonte única de verdade** para implementação de segurança no sistema IAFÉ IMOBI. Baseado em auditoria completa realizada em 2025-01-27, identifica vulnerabilidades críticas e apresenta plano detalhado de correção.
...
### **Objetivo Principal**
Transformar o IAFÉ IMOBI em um sistema comercialmente seguro, eliminando todas as vulnerabilidades críticas e implementando as melhores práticas de segurança para aplicações SaaS.
...
SELECT vault.create_secret('n8n_webhook_secret', '<DEFINIR_SENHA_FORTE>');
SELECT vault.create_secret('evolution_api_token', '<DEFINIR_TOKEN>');
SELECT vault.create_secret('default_user_password', '<DEFINIR_SENHA_FORTE>');
SELECT vault.create_secret('smtp_password', '<DEFINIR_SENHA_SMTP>');
SELECT vault.create_secret('openai_api_key', '<DEFINIR_CHAVE_OPENAI>');
```
...
SUPABASE_URL=https://bfcssdogttmqeujgmxdf.supabase.co
