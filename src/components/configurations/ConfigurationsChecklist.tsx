...
export function ConfigurationsChecklist({ items }: Props) {
  return (
    <div className="rounded-2xl border border-border/70 bg-white dark:bg-card p-4 sm:p-5 shadow-sm space-y-3">
      <h3 className="text-sm font-semibold text-foreground">Cadastro completo?</h3>
      <ul className="space-y-2.5">
        {items.map((item) => (
          <li key={item.id} className="flex items-center justify-between gap-3">
            ...
            <span className="text-xs text-muted-foreground shrink-0 tabular-nums">{item.detail}</span>
          </li>
        ))}
      </ul>
      <p className="text-xs text-muted-foreground leading-relaxed pt-1 border-t border-border/60">
        CNPJ e endereço completos liberam emissão de contratos direto da plataforma.
      </p>
    </div>
  );
}
