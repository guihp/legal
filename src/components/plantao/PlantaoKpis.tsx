...
import type { PlantaoKpiItem } from './helpers';

type Props = {
  items: PlantaoKpiItem[];
};

export const PlantaoKpis = memo(function PlantaoKpis({ items }: Props) {
  if (!items.length) return null;

  return (
    <div className="flex gap-2 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden pb-0.5 -mx-0.5 px-0.5 snap-x snap-mandatory md:mx-0 md:grid md:grid-cols-2 md:gap-2.5 md:overflow-visible md:px-0 md:pb-0 md:snap-none xl:grid-cols-4">
      {items.map((kpi) => (
        <div
          key={kpi.key}
          ...
          {kpi.hint ? (
            <p
              className={cn(
                ...
                kpi.hintTone === 'negative' && 'text-amber-700 dark:text-amber-400',
