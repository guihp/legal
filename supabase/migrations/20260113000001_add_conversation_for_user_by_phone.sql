...
BEGIN
  ...
  RETURN QUERY EXECUTE format('
    ...
    allowed_sessions AS (
      SELECT DISTINCT m.session_id::text as sid
      FROM public.%I m
      CROSS JOIN me
      WHERE m.session_id::text = $1
        AND (
          me.role IN (''admin'',''gestor'')
          OR (me.role = ''corretor'' AND (
            m.instancia = me.chat_instance
            OR EXISTS (
              SELECT 1 FROM public.leads l
              WHERE l.id::text = m.session_id::text
                AND l.company_id = me.company_id
                AND l.id_corretor_responsavel = me.uid
            )
          ))
        )
    ),
