-- Nome/foto do lead nas listas de conversa (WhatsApp + Instagram) para quem não passa no RLS de leads (ex.: corretor).
-- SECURITY DEFINER: admin/gestor/super_admin veem dados do lead; corretor só mesma empresa e (lead.user_id is null ou lead.user_id = auth.uid()).

DROP FUNCTION IF EXISTS public.list_conversations_by_phone(text, text);
DROP FUNCTION IF EXISTS public.list_instagram_conversations(uuid, text);
DROP FUNCTION IF EXISTS public.list_conversations_by_instagram_company(uuid, text);
